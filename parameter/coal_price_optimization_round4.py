...
"""
煤炭价格预测 - 第四轮参数优化
基于第三轮最优配置(Round3_Config2_MultiHead8)进行精细化调优
重点探索8头注意力的最优参数组合
"""
...
# 参数搜索空间 - 基于Round3_Config2_MultiHead8优化
PARAM_GRID = [
    # 配置1: 基准复现(8头)
    {
        'name': 'Round4_Config1_Baseline8Head',
        'sequence_length': 60,
        'lstm_units': 384,
        'lstm_units_2': 192,
        'lstm_units_3': 96,
    ...
    # 配置2: 8头 + 更大Attention维度(320)
    {
        'name': 'Round4_Config2_8Head_Attn320',
        'sequence_length': 60,
        'lstm_units': 384,
        'lstm_units_2': 192,
        'lstm_units_3': 96,
        'attention_dim': 320,  # 256→320, 每头40维
        'num_attention_heads': 8,
        'dropout_rate': 0.4,
        'l2_reg': 0.001,
        'learning_rate': 0.00015,
    ...
    # 配置3: 8头 + 降低正则化
    {
        'name': 'Round4_Config3_8Head_LowerReg',
        'sequence_length': 60,
        'lstm_units': 384,
        'lstm_units_2': 192,
        'lstm_units_3': 96,
    ...
    # 配置4: 8头 + 更大LSTM容量
    {
        'name': 'Round4_Config4_8Head_BiggerLSTM',
        'sequence_length': 60,
        'lstm_units': 448,     # 384→448
        'lstm_units_2': 224,   # 192→224
        'lstm_units_3': 112,   # 96→112
        'attention_dim': 256,
        'num_attention_heads': 8,
        'dropout_rate': 0.42,  # 增加dropout防过拟合
        'l2_reg': 0.0012,
        'learning_rate': 0.00015,
        'epochs': 400,
        'batch_size': 32,
    ...
    # 配置5: 8头 + 学习率微调
    {
        'name': 'Round4_Config5_8Head_LR00018',
        'sequence_length': 60,
        'lstm_units': 384,
        'lstm_units_2': 192,
        'lstm_units_3': 96,
        ...
        'dropout_rate': 0.4,
        'l2_reg': 0.001,
        'learning_rate': 0.00018,  # 0.00015→0.00018
        'epochs': 400,
        'batch_size': 32,
    },
    
    # 配置6: 8头 + 最优组合(Attn320 + LowerReg)
    {
        'name': 'Round4_Config6_8Head_Optimal',
        'sequence_length': 60,
        'lstm_units': 384,
        'lstm_units_2': 192,
        'lstm_units_3': 96,
        'attention_dim': 320,  # 更大维度
        'num_attention_heads': 8,
        'dropout_rate': 0.38,  # 中等正则
        'l2_reg': 0.0009,
        'learning_rate': 0.00015,
        'epochs': 400,
        'batch_size': 32,
...
def run_optimization():
    ...
    print(f"基于第三轮最优Round3_Config2_MultiHead8 (R²=0.358)")
    print(f"核心改进: 8头注意力精细调优")
    print("\n" + "="*80 + "\n")
    ...
    output_path = f'/Users/Jason/Desktop/code/AI/parameter/coal_price_optimization_round4_{timestamp}.xlsx'
    
    with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
        results_df.to_excel(writer, sheet_name='完整结果', index=False)
        
        top3 = results_df.head(3)[['config_name', 'R2', 'RMSE', 'MAPE', 
                                    'Direction_Accuracy', 'attention_dim', 
                                    'num_attention_heads', 'learning_rate']]
    ...
    print("="*80)
    print(f"第四轮优化完成! 最佳R² = {results_df.iloc[0]['R2']:.4f}")
    print(f"相比第三轮(R²=0.358): {(results_df.iloc[0]['R2']-0.358)/0.358*100:+.1f}%")
    print("="*80 + "\n")
